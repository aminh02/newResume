<?php /* Copyright 2017-2022 RAGE Software Inc. All Rights Reserved */ if (!isset($_POST)) {
...
{
    if (filter_var($ip, FILTER_VALIDATE_IP,  FILTER_FLAG_IPV4 |  FILTER_FLAG_IPV6 | FILTER_FLAG_NO_PRIV_RANGE |  FILTER_FLAG_NO_RES_RANGE) === false) return false;
}
$newline = "\r\n";
$emailContent = '';
$emailSender = '';
...
$file_url = "";
$customer_code = "";
$form_code = "0D3F270DE7FE465BB5911E39B9A57303F0B047EBBD1D4B0C80E97764A64F7E14";
$form_name = "My Contact Form";
$emailRecipient = "hadavi@sharif.edu";
$confirmationEmail = "Hi %name%, 
...
if ($customer_code != "") {
    if (isset($_POST['required_size'])) {
        ...
        if ($numFiles > 0) {
            $intMemLimit = return_bytes(ini_get('memory_limit'));
            $intPostMaxSize = return_bytes(ini_get('post_max_size'));
            $intMaxFileLimit = return_bytes(ini_get('max_file_uploads'));
            $intMaxEachFileSize = return_bytes(ini_get('upload_max_filesize'));
            $intMaxTotalFileSizeAllowed = min($intMemLimit, $intPostMaxSize);
            ...
            if ($required_size > $intMaxTotalFileSizeAllowed) {
...
if ($check_required_size !== FALSE) {
    if (isset($_POST['g-recaptcha-response'])) {
        $recaptcha_key = "";
        $recaptcha = new EverWeb_ReCaptcha($recaptcha_key);
...
$emailContent .= "<!--placeholder-warnings-->";
$emailContent .= '<table style="width:100%;" cellpadding="10" cellspacing="0">' . $newline;
$emailContent .= "<tr>";
...
foreach ($_POST as $key => $value) {
    if (!in_array($key, $ignoredFields) && strpos($key, '_label') === FALSE) {
        ...
        $formField = htmlspecialchars($_POST[$key . '_label']);
        if ($formField == "") {
        } else {
            $okToSend = TRUE;
            $formField = urldecode($formField);
            $row++;
            if ($row % 2 == 0) {
                $emailContent .= "<tr>";
            } else {
            ...
            $formPlainText .= "> " . $formField . ": ";
            $emailContent .= "</td>" . $newline;
            $emailContent .= "<td style=\"border-top:1px solid #c0c0c0\">" . $newline;
            $finalValue = "";
            if (is_array($value)) {
...
$strEmailAttachments .= "</td>" . $newline;
$strEmailAttachments .= "<td style=\"border-top:1px solid #c0c0c0\">" . $newline;
...
$formPlainText = "\n\n" . $formPlainText;
$formPlainText = rawurlencode($formPlainText);
...
if ($check_required_size !== FALSE) {
    if ($customer_code != "") {
        ...
        if (!is_array($stats)) {
            ...
            if ($stats['submissions_used'] < $stats['submissions_quota']) {
                $can_submit = TRUE;
            } else {
                $can_submit = FALSE;
            }
        ...
        json(['success' => TRUE, 'can_submit' => FALSE, 'can_upload' => FALSE]);
        exit;
    }
}
$emailStats = "";
if ($customer_code != "") {
    ...
    $space_used = $_POST['space_used'];
    $space_quota = $_POST['space_quota'];
    $errorMsg = "";
    $numAttachments = 0;
...
$password  = "";
$protocol = "ssl";
$strConfirmationSubject = "Thank you for your submission";
...
if ($okToSend) {
    ...
    if (!$mail->sendMail($emailSender, $emailRecipient, $emailSubject, $emailContent)) {
        json(['success' => FALSE, 'message' => "Could not send email: " . $mail->errorMessage, 'log' => htmlentities($mail->printDebugLog(TRUE))]);
    } else {
        if (!$doSendConfirmation) {
            json(['success' => TRUE, 'message' => "Email sent successfully", 'url' => $file_url]);
        } else {
            $mail = new EverWeb_Mail($server, $port, $username, $password, $protocol);
            $mail->contentType = "text/plain";
            if (!$mail->sendMail($emailRecipient, $emailSender,  "Thank you for your submission", $confirmationEmail, null, true, "text/plain")) {
